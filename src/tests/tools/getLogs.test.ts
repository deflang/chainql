...
describe("getLogs", () => {
  ...
  it("rejects invalid combination of blockHash and from/to block", async () => {
    ...
    expect(result.content?.[0]?.text).toContain(
      "blockHash cannot be combined"
    );
  });

  it("fetches logs by blockHash", async () => {
    const topic = "0x241ea03ca20251805084d27d4440371c34a0b85ff108f6bb5611248f73818b80";
    const mockLogs = [
      {
        address: "0x1",
        topics: [topic],
  ...
  it("handles network errors", async () => {
    ...
    const result = await getLogs.handler({ fromBlock: "latest", toBlock: "latest" });

    expect(result.content?.[0]?.text).toContain("Error fetching logs: Network down");
  });
});
