...
export const getUncleCountByBlockNumber = {
  name: "eth_get_uncle_count_by_block_number",
  description:
    "Returns the number of uncles in a block matching the given block number or tag. Defaults to Ethereum mainnet if no chain is specified.",
  schema: {
    blockNumberOrTag: z
      .string()
      .describe(
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Uncle count for block ${blockNumberOrTag} on chainid ${selectedChainId}: ${count} (hex: ${data.result})`,
          },
        ],
      };
    } catch (err: unknown) {
      ...
      return {
        content: [
          { type: "text", text: `Error fetching uncle count: ${message}` },
        ],
      };
    }
  },
