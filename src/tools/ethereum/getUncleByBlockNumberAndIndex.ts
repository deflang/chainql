...
export const getUncleByBlockNumberAndIndex = {
  name: "eth_get_uncle_by_block_number_and_index",
  description:
    "Returns information about an uncle of a block given the block number or tag and the uncle index position. Defaults to Ethereum mainnet if no chain is specified.",
  schema: {
    blockNumberOrTag: z
      .string()
      .describe(
        "Hex block number (e.g. 0x29c) or tag (latest, earliest, pending, safe, finalized)"
      ),
    ...
    try {
      ...
      if (data.result === null) {
        return {
          content: [
            {
              type: "text",
              text: `No uncle found at index ${index} for block ${blockNumberOrTag} on chainid ${selectedChainId}`,
            },
          ],
        };
      }
      ...
      return {
        content: [
          {
            type: "text",
            text: `Uncle for block ${blockNumberOrTag} index ${index} on chainid ${selectedChainId}`,
          },
