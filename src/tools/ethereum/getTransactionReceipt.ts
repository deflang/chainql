...
export const getTransactionReceipt = {
  ...
  description:
    "Returns the receipt of a transaction by hash. Defaults to Ethereum mainnet if no chain is specified.",
  schema: {
    ...
    try {
      ...
      if (data.result === null) {
        return {
          content: [
            {
              type: "text",
              text: `No transaction receipt found for hash ${hash} on chainid ${selectedChainId}`,
            },
          ],
        };
      }
      ...
      return {
        content: [
          {
            type: "text",
            text: `Transaction receipt for hash ${hash} on chainid ${selectedChainId}`,
          },
      ...
      return {
        content: [
          { type: "text", text: `Error fetching transaction receipt: ${message}` },
        ],
      };
    }
  },
